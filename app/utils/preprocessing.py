...
def get_image_paths(data_folder_path):
    ...
    for dir_name in dirs:
        ...
        print(f"Skipping non-image file: {file_path}")  # CSV or any other non-image file
...
# Example usage
src = "data/train/GTSRB/Training"
# images, labels, image_paths = readTrafficSigns(src)
labels, images = get_images_and_labels(data_folder_path=src)
# print(len(images))
# image_paths = get_image_paths(data_folder_path=src)
# print(len(image_paths))
test_src = "data\\test\\GTSRB\\Final_Test"
images_path =  get_image_paths(test_src)
imagest= get_images(images_path)
# print(len(imagest))
...
def safe_preprocessing(images):
    ...
    for image in images:
        ...
        image_equalized = cv2.equalizeHist(image_resized)
        edited_images.append(image_equalized)  # Safe uint8
    return edited_images

train_edited_images = safe_preprocessing(images)
test_edited_images = safe_preprocessing(imagest)
...
analysis = analyze_dataset(labels, images)
